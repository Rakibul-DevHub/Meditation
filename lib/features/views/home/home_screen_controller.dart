...
class HomeScreenController extends GetxController {
  ...
  // ✅ NEW METHOD: Refresh data after sign-in
  Future<void> refreshAfterSignIn() async {
    debugPrint('🔄 Refreshing home data after sign-in');
    ...
    // Fetch fresh data with loading indicators
    await Future.wait([
      fetchFeaturedSounds(showLoading: true, isRefresh: false),
      fetchSleepTonight(showLoading: true, isRefresh: false),
      fetchPopularSounds(showLoading: true, isRefresh: false),
    ]);

    debugPrint('✅ Home data refreshed after sign-in. Featured: ${featuredTracks.length}, Sleep: ${sleepTonightTracks.length}, Popular: ${popularTracks.length}');
  }
