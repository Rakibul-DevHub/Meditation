...
class _NowPlayingScreenState extends State<NowPlayingScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _initDownloadState();
      _fetchUserSleepTimer(); // Fetch saved sleep timer from server
    });
  }
  ...
  // ── Sleep Timer API Methods ───────────────────────────────────────────────

  /// Fetch user profile to get saved sleep timer value
  Future<void> _fetchUserSleepTimer() async {
    try {
      ...
      if (response.isSuccess && response.jsonResponse != null) {
        ...
        final serverSleepTimer = data['sleepTimer'];
        if (serverSleepTimer != null) {
          // Convert server value to display label
          String displayLabel = 'Off';
          if (serverSleepTimer is String) {
            if (serverSleepTimer == '0') {
              displayLabel = 'Off';
            } else if (serverSleepTimer == '15') {
              displayLabel = '15 min';
            } else if (serverSleepTimer == '30') {
              displayLabel = '30 min';
            } else if (serverSleepTimer == '45') {
              displayLabel = '45 min';
            } else if (serverSleepTimer == '60') {
              displayLabel = '1 hour';
            }
          } else if (serverSleepTimer is int) {
            if (serverSleepTimer == 0) {
              displayLabel = 'Off';
            } else if (serverSleepTimer == 60) {
              displayLabel = '1 hour';
            } else {
              displayLabel = '${serverSleepTimer} min';
            }
          }
          setState(() {
            selectedSleepTimer = displayLabel;
          });
          _applySleepTimer(_labelToMinutes(displayLabel));
        }
      }
    } catch (e) {
      debugPrint('Error fetching sleep timer: $e');
    }
  }
  ...
  /// Update sleep timer on the server
  Future<void> _updateSleepTimer(String label) async {
    // Store previous value for rollback
    final previousLabel = selectedSleepTimer;
    setState(() {
      selectedSleepTimer = label;
    });

    // Convert label to minutes
    int minutes = _labelToMinutes(label);

    try {
      final token = await SecureStorageService.instance.getAccessToken();
      if (token == null || token.isEmpty) {
        setState(() {
          selectedSleepTimer = previousLabel;
        });
        return;
      }

      // ✅ FIX: Send as string instead of integer
      final Map<String, dynamic> body = <String, dynamic>{};
      String sleepTimerValue = minutes == 0 ? '0' : minutes.toString();
      body['sleepTimer'] = sleepTimerValue;

      debugPrint('📤 Sending sleep timer to API: "$sleepTimerValue" (as String)');
      ...
      if (response.isSuccess) {
        debugPrint('✅ Sleep timer saved: $sleepTimerValue');

        // Show confirmation message
        String message = label == 'Off'
            ? 'Sleep timer turned off'
            : 'Sleep timer set to $label';
        ...
        // Apply the sleep timer functionality
          ...
          children: [
            ...
            ...timers.map((t) => ListTile(
              ...
              onTap: () {
                ...
                _updateSleepTimer(t); // Call API to save
              },
            )),
            const SizedBox(height: 10),
          ],
